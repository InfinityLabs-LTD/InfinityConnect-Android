...
class InfinityVpnService : VpnService() {
    ...
    companion object {
         ...
         *
         * ВЫКЛЮЧЕН по умолчанию (fail-open): при обрыве туннель снимается
         * полностью и трафик идёт напрямую. Причина — предсказуемость для
         * пользователя: при fail-closed интернет пропадает «молча», а понять,
         * что его блокирует именно наше приложение, из UI сейчас негде (в
         * главном экране состояние kill-switch отдельно не показано). Включать
         * стоит вместе с явной индикацией «трафик заблокирован» в UI — тогда
         * поведение перестаёт выглядеть поломкой сети.
         *
         * Константа, а не настройка: поведение должно быть предсказуемым, но
         * переключатель оставлен, чтобы включить fail-closed без хирургии по
         * коду. Вся логика удержания TUN написана и рабочая — см.
         * [failWithKillSwitch] и releaseActiveTunnel(keepTun).
         */
        private const val KILL_SWITCH_ENABLED = false
